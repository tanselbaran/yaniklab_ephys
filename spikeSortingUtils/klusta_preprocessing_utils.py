...
def create_prm_file(probe,s,p):
    ...
    filter_low = p['low_cutoff']
    extract_s_before = p['samples_before']
    extract_s_after = p['samples_after']
    with open(file_dir, 'a') as text:
        ...
        print('traces = dict(raw_data_files=[experiment_name + \'.dat\'],voltage_gain=10., sample_rate =' + str(p['sample_rate']) + ', n_channels = ' + str(p['nr_of_electrodes_per_group']) + ', dtype = \'int16\')', file = text)

        print("spikedetekt = { \n 'filter_low' : %d., \n 'filter_high_factor': 0.95 * .5, \n 'filter_butter_order': 3, \n #Data chunks. \n 'chunk_size_seconds': 1., \n 'chunk_overlap_seconds': .015, \n #Threshold \n 'n_excerpts': 50, \n 'excerpt_size_seconds': 1., \n 'use_single_threshold': True, \n 'threshold_strong_std_factor': 4.5, \n 'threshold_weak_std_factor': 2., \n 'detect_spikes': 'negative', \n # Connected components. \n 'connected_component_join_size': 1, \n #Spike extractions. \n 'extract_s_before': %i, \n 'extract_s_after': %i, \n 'weight_power': 2, \n #Features. \n 'n_features_per_channel': 3, \n 'pca_n_waveforms_max': 10000}" % (filter_low, extract_s_before, extract_s_after), file = text)
...
def create_prb_file(probe,s,p):
    ...
    if p['probe_type'] == 'tetrode':
        tetrode = dict(channels = list(range(p['nr_of_electrodes_per_group'])), graph = list(combinations(range(p['nr_of_electrodes_per_group']),2)), geometry = {0: (0, 90), 1: (0, 60), 2: (0, 30), 3: (0, 0)})
